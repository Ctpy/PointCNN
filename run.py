...
from dataloader.DLODataloader import DLOModule
...
def train_process():
    ...
    elif config.dataset == "dlo_dataset":
        dl = DLOModule("F:\wire_hareness_dataset\data\labels\set2")
        dl.setup("fit")
        train_set = dl.train_dataloader()
        valid_set = dl.val_dataloader()
    else:
    ...
    if config.train.optimizer == 'SGD':
        ...
        optimizer = optim.Adam(net.parameters(), lr=config.train.learning_rate_base, eps=config.train.epsilon,
...
def create_model(base_model, ckpt_file=None, from_measurement=None):
    ...
    elif base_model == 'dlo_x3_2048':
        net = PointCNN.dlo_x3_2048()
    else:
        raise NotImplementedError
...
def train_epoch(data_loader, net: nn.Module, criterion, optimizer, epoch):
    ...
    for i, sample in enumerate(data_loader):
        ...
        lr = config.train.learning_rate_base * (
            math.pow(config.train.decay_rate, global_step // config.train.decay_steps))
        ...
        xforms, rotations = pf.get_xforms(config.train.batch_size, config.dataset_setting["rotation_range"],
...
def evaluate(data_loader, net: nn.Module, calc_confusion_matrix=False, rtn_features=False):
    ...
    for i, sample in enumerate(data_loader):
        ...
        if config.task == "cls":
            ...
            batch_labels = batch_labels.view(-1, 1)[indices].view(shape[0], config.dataset_setting["sample_num"])
        features_augmented = None
        xforms, rotations = pf.get_xforms(shape[0], config.dataset_setting["rotation_range_val"],
