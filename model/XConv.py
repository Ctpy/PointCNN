...
class XConv(torch.nn.Module):
    ...
    def __init__(self, setting, idx, K, dilation, P, C, C_pts_fts, depth_multiplier, with_X_transfomation,
        ...
        if self.with_X_transformation:
            ...
            self.add_module(name='x_trans_depthConv1', module=torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=self.K, out_channels=self.K * self.K, kernel_size=(1, self.K), groups=K,
                                bias=False),
        ...
        if self.idx < len((self.setting["xconv_params"])):
            ...
            in_channels_fts_conv = self.C_pts_fts + self.setting["xconv_output_dim"][
                self.setting["xdconv_params"][self.idx - len((self.setting["xconv_params"]))]["qrs_layer_idx"] + 1]

        self.add_module(name="fts_conv", module=torch.nn.Sequential(torch.nn.Conv2d(in_channels=in_channels_fts_conv,
                                                                                    out_channels=in_channels_fts_conv * self.depth_multiplier,
                                                                                    kernel_size=(1, self.K),
                                                                                    groups=in_channels_fts_conv,
                                                                                    bias=False).to(
            device=config.device),
                                                                    torch.nn.Conv2d(
                                                                        in_channels=self.depth_multiplier * in_channels_fts_conv,
                                                                        out_channels=self.C, kernel_size=(1, 1),
                                                                        bias=False)
                                                                    .to(device=config.device),
                                                                    torch.nn.ELU().to(device=config.device)).to(
            device=config.device))
        self.add_module(name="fts_conv_BN", module=torch.nn.BatchNorm1d(num_features=self.C))
        if self.with_global:
            self.add_module(name="dense3",
                            module=torch.nn.Sequential(torch.nn.Linear(3, self.C // 4).to(device=config.device),
                                                       torch.nn.ELU().to(device=config.device),
                                                       View(shape=(-1, self.C // 4)),
                                                       torch.nn.BatchNorm1d(num_features=self.C // 4).to(
                                                           device=config.device), \
                                                       torch.nn.Linear(self.C // 4, self.C // 4).to(
                                                           device=config.device),
                                                       torch.nn.ELU().to(device=config.device),
                                                       torch.nn.BatchNorm1d(num_features=self.C // 4).to(
                                                           device=config.device),
    ...
    def forward(self, pts, fts, qrs):
        ...
        indices = (indices.view(-1, 2)[:, 1].cpu() + torch.arange(0, N * point_num, point_num).view(-1, 1).repeat(1,
                                                                                                                  self.P * self.K).view(
            -1)).cpu().numpy()
        if self.sorting_method is not None:
            raise NotImplementedError

        nn_pts = (pts.contiguous().view(-1, 3))[indices].view(N, self.P, self.K,
                                                              3)  # coordinates of nearest-neighbour points
        ...
        if fts is None:
            ...
            nn_fts_from_prev = (fts.contiguous().view(N * point_num, -1))[indices].contiguous().view(N, self.P, self.K,
                                                                                                     -1)  # the F matrix
        ...
        fts_conv_3d = self._modules['fts_conv'].forward(fts_X.transpose(1, 3).transpose(2, 3)).transpose(1,
                                                                                                         2).contiguous().view(
            -1, self.C)
